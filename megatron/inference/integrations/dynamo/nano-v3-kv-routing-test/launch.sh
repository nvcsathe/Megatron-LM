...
ROUTER_KV_OVERLAP_SCORE="${ROUTER_KV_OVERLAP_SCORE:-1}"
...
if [[ "$PREFIX_CACHE_EVICTION_POLICY" != "lru" ]]; then
    echo "PREFIX_CACHE_EVICTION_POLICY must be lru for sequential KV-reuse tests" >&2
    exit 2
fi
...
export ROLE_EP_SIZE GPUS_PER_NODE KV_BLOCK_SIZE PREFIX_CACHE PREFIX_CACHE_EVICTION_POLICY STAGE
export ROUTER_KV_OVERLAP_SCORE
...
EXPORT_VARS="$EXPORT_VARS,KV_BLOCK_SIZE,MAMBA_GB,PREFIX_CACHE,PREFIX_CACHE_EVICTION_POLICY,CUDA_GRAPH_IMPL,MODEL_ARGS_OVERRIDE"
EXPORT_VARS="$EXPORT_VARS,ROUTER_KV_OVERLAP_SCORE"
...
echo "[launch] KV policy:  $PREFIX_CACHE_EVICTION_POLICY"
echo "[launch] KV score:   $ROUTER_KV_OVERLAP_SCORE"
