...
"""Pure index arithmetic for KV re-sharding across heterogeneous parallelism.

No NIXL, no torch required. Functions here compute *what* to pull (which peer,
which block ids, which outer-slice range, which head range); ``kv_transfer.py``
...
class TransferSegment:
    ...
    ``kv_transfer.KvTransferAgent._execute_segment`` turns this into NIXL
...
def build_reshard_plan(
    ...
    ``KvTransferAgent._execute_segment``.
    """
