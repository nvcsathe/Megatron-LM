...
"""Mamba-aware flow control for coordinator-native disaggregation."""
...
class DisaggMambaFlowControl:
    ...
    def _capacity_from_instance_meta(instance_meta) -> Optional[int]:
        ...
        capacities = [
            int(entry["mamba_slot_capacity"])
            for entry in entries
            if entry.get("mamba_slot_capacity") is not None
        ]
        if capacities and len(capacities) != len(entries):
            raise ValueError(
                "Mamba durable slot capacity is missing from part of an engine's "
                "model-parallel metadata"
            )
    ...
    def try_reserve(self, identity, request_id: int, slot_cost: int) -> bool:
        ...
        if request_id in self._decode_reservations:
            raise RuntimeError(f"Decode request {request_id} already holds a Mamba reservation")
    ...
    def release_decode(self, request_id: int):
        ...
        if usage < slot_cost:
            raise RuntimeError(
                f"Decode Mamba slot accounting underflow on {identity!r}: "
                f"used={usage}, release={slot_cost}"
            )
        self._decode_usage[identity] = usage - slot_cost
        return identity
