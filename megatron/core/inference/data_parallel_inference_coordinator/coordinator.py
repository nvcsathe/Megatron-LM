...
class DataParallelInferenceCoordinator:
    ...
    def _remove_engine(self, identity):
        ...
        if idx is not None:
            ...
            for ident in identity_to_rank_index:
                if identity_to_rank_index[ident] > idx:
                    identity_to_rank_index[ident] -= 1
            # Drop hash-table entries for the removed rank; shift indices above it.
            new_hash_table = {}
            for block_hash, rank_timestamps in self._hash_table.items():
                new_row = {}
                for rank_idx, timestamp in rank_timestamps.items():
                    if rank_idx == idx:
                        continue
                    new_idx = rank_idx - 1 if rank_idx > idx else rank_idx
                    new_row[new_idx] = timestamp
                if new_row:
                    new_hash_table[block_hash] = new_row
