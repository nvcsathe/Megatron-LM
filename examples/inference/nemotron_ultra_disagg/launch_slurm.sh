...
export INFERENCE_MAX_REQUESTS="${INFERENCE_MAX_REQUESTS:-8}"
...
if (( INFERENCE_MAX_REQUESTS <= 0 || INFERENCE_MAX_REQUESTS % TP_SIZE != 0 )); then
    echo "INFERENCE_MAX_REQUESTS must be positive and divisible by TP_SIZE=${TP_SIZE}; got ${INFERENCE_MAX_REQUESTS}." >&2
    exit 2
fi
