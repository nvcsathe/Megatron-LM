...
#     NATS + etcd
#     │
#     ├── megatron.inference.integrations.dynamo owned engine [PREFILL] (TP/PP rank group)
#     ├── megatron.inference.integrations.dynamo owned engine [DECODE]  (TP/PP rank group)
#     └── dynamo.frontend
#
# Outputs:
...
(
    CUDA_VISIBLE_DEVICES="$PREFILL_GPUS" exec python -m megatron.inference.integrations.dynamo \
            --role prefill \
...
(
    CUDA_VISIBLE_DEVICES="$DECODE_GPUS" exec python -m megatron.inference.integrations.dynamo \
            --role decode \
            --model "$TOKENIZER_MODEL" \
            --served-model-name "$SERVED_MODEL_NAME" \
