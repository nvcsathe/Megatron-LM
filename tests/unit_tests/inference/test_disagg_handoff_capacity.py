...
def test_capacity_miss_defers_before_any_transfer():
    loop = asyncio.new_event_loop()
    try:
        engine = _HandoffHarness(loop, available=1)
...
def test_attention_only_handoff_has_no_mamba_admission_overhead():
    loop = asyncio.new_event_loop()
    try:
        engine = _HandoffHarness(loop, available=0)
...
def test_capacity_queue_is_fifo():
    loop = asyncio.new_event_loop()
    try:
        engine = _HandoffHarness(loop, available=1)
        ...
        _drain_loop(loop)
        assert not engine._deferred_kv_handoffs
        assert [call[0]["request_id"] for call in engine._kv_transfer_agent.calls] == [
            1,
            2,
        ]
    finally:
...
def test_peer_capacity_miss_rolls_back_before_any_transfer(monkeypatch):
    loop = asyncio.new_event_loop()
    try:
        engine = _HandoffHarness(loop, available=2)
...
def test_reset_cancels_capacity_queued_handoffs():
    loop = asyncio.new_event_loop()
    try:
        engine = _HandoffHarness(loop, available=0)
        future = engine.add_request_with_kv_handoff(
